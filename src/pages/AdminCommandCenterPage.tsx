      ...
      <section className="command-center-grid" aria-label={language === 'th' ? 'ศูนย์ควบคุมระบบ' : 'Admin command center'}>
        ...
        <Card className="command-panel">
          ...
          <div className="admin-quick-action-grid">
            ...
            <Link className="admin-quick-action-card" to="/admin/staff/attendance"><ClipboardCheck size={22} /><strong>{language === 'th' ? 'เช็กชื่อทีมงาน' : 'Staff Attendance'}</strong><span>{language === 'th' ? 'รอบเช็กชื่อและ QR' : 'Sessions and QR tools.'}</span></Link>
